...
Presentation  = 'Optimize de function 3x0*x1 + sin(x2)*(2x3-x4+x5) + (x6-x1)^(x4*x7-x0*x8+1)'
...
def MathFunction3(solution):  # version of OneMax counting the presence of MAX_VALUE (1 as a particular case)
    # 3x0*x1 + sin(x2)*(2x3-x4+x5) + (x6-x1)^(x4*x7-x0*x8+1)
    fitness = 3*solution[0]*solution[1] + \
              math.sin(solution[2])*(2*solution[3]-solution[4]+solution[5]) + \
              math.pow(solution[6]-solution[1], solution[4]*solution[7]-solution[0]*solution[8]+1)
    return fitness
