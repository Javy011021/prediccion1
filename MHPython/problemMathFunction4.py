...
Presentation  = 'Optimize de function 3x0*x1 + sin(x2)*(2x3-x4+x5) + (x6-x1)^(x4*x7-x0*x8+100)'
...
def MathFunction4(solution):  # version of OneMax counting the presence of MAX_VALUE (1 as a particular case)
    # 3x0*x1 + sin(x2)*(2x3-x4+x5) + (x6-x1)^(x4*x7-x0*x8+100)
    # Solution:[9, 0, 9, 9, 9, 9, 9, 9, 0, 9] with evaluation 5.222689519770958e+172
    fitness = 3*solution[0]*solution[1] + \
              math.sin(solution[2])*(2*solution[3]-solution[4]+solution[5]) + \
              math.pow(solution[6]-solution[1], solution[4]*solution[7]-solution[0]*solution[8]+100)
    return fitness
